$(document).ready(function () {
  ...
  function loadStudentsByGroup(groupId) {
      $.getJSON(`http://localhost:8000/users_by_group/${groupId}`, function (data) {
          ...
          content += "<thead class=''><tr><th class='index-col'>№</th><th class='last-name-col'>Фамилия</th><th class='first-name-col'>Имя</th><th class='middle-name-col'>Отчество</th><th class='edit-col'>Редактирование</th><th class='delete-col'>Удаление</th></tr></thead><tbody>";
          data.students.forEach(function (student, index) {
              content += "<tr>";
              content += "<td class='index-col'>" + (index + 1) + "</td>";
          ...
          $(".delete-btn").click(function () {
              ...
              var userId = data.students[index].User_ID;
  ...
  $("#saveBtn").click(function () {
      var index = $("#editStudentIndex").val();
      var student = data.students[index];
      ...
      $.ajax({
          url: "http://localhost:8000/students/" + student.User_ID,
          type: "PUT",
          contentType: "application/json",
          data: JSON.stringify(data),
          success: function () {
              $("#editModal").modal('hide');
              loadStudentsByGroup(data.group_id);
              alert("Информация о студенте успешно обновлена!");
          },
          error: function () {
              alert("Произошла ошибка при обновлении информации о студенте.");
          }
      });
  });

  // Обработчик кнопки "Показать студентов"
