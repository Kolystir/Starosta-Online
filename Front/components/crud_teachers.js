$(document).ready(function () {
    ...
    function loadGroups() {
        $.getJSON("http://localhost:8000/group", function (data) {
            ...
            groupSelect.empty();
            groupSelect.append("<option value=''>Без группы</option>");
    ...
    function createTeacher(event) {
        ...
        var groupId = $("#group-select").val() || null; // Передаем null, если группа не выбрана
        var teacherData = {
        ...
        console.log("Отправка данных:", teacherData); // Логируем данные перед отправкой
        $.ajax({
            url: url,
            type: type,
            ...
            error: function (xhr, status, error) {
                console.error("Ошибка при создании учителя:", xhr.responseText);
                if (
                    xhr.status === 422 &&
                    xhr.responseJSON &&
                    ...
                    console.error("Ошибка при создании учителя:", xhr.responseText);
                    $("#error-message")
                        .text("Произошла ошибка при создании учителя")
                        .show();
                }
            },
        });
    ...
    function updateTeacher() {
        ...
        $.ajax({
            ...
            error: function (xhr, status, error) {
                console.error("Ошибка при обновлении учителя:", xhr.responseText);
            },
        });
    }
